...
namespace ExpressVoitures.Areas.Identity.Pages.Account.Manage
{
    public class ExternalLoginsModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetLinkLoginCallbackAsync()
        {
            ...
            if (info == null)
            {
                throw new InvalidOperationException($"Une erreur inattendue s’est produite lors du chargement des informations de connexion externes.");
            }

            var result = await _userManager.AddLoginAsync(user, info);
            if (!result.Succeeded)
            {
                StatusMessage = "Le login externe n’a pas été ajouté. Les connexions externes ne peuvent être associées qu’à un seul compte.";
                return RedirectToPage();
            }
